          ...
          {navItems.map((tab) => (
            <Link href={tab.path || "#"} key={tab.id} className="block">
              <Button
                ...
                className={`w-full flex items-center justify-start rounded-lg transition-colors ${isCollapsed ? "p-2 px-3" : "px-4 py-2"} ${activeTab === tab.id ? "bg-orange-50 text-[#F06A13] hover:bg-orange-50" : "text-gray-800 hover:bg-gray-200"}`}
                title={isCollapsed ? tab.name : undefined}
              >
                <div
                  className={`flex items-center ${activeTab === tab.id ? "text-[#F06A13]" : "text-gray-500"} ${!isCollapsed ? "mr-2" : ""}`}
                >
